...
export async function generateStaticParams() {
  const articles = getArticlesByCategory("denial-reasons");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = getSeoArticle("denial-reasons", slug);
  if (!article) return {};
...
export default async function DenialReasonArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const article = getSeoArticle("denial-reasons", slug);
