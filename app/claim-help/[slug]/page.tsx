...
export async function generateStaticParams() {
  const articles = getArticlesByCategory("claim-help");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = getSeoArticle("claim-help", slug);
  if (!article) return {};
...
export default async function ClaimHelpArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const article = getSeoArticle("claim-help", slug);
