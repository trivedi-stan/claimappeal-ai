...
export async function generateStaticParams() {
  const articles = getArticlesByCategory("claim-denial");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = getSeoArticle("claim-denial", slug);
  if (!article) return {};
...
export default async function ClaimDenialArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const article = getSeoArticle("claim-denial", slug);
