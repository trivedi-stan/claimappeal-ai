...
export async function generateStaticParams() {
  const articles = getArticlesByCategory("templates");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = getSeoArticle("templates", slug);
  if (!article) return {};
...
export default async function TemplateArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const article = getSeoArticle("templates", slug);
