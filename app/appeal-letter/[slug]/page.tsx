...
export async function generateStaticParams() {
  const articles = getArticlesByCategory("appeal-letter");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = getSeoArticle("appeal-letter", slug);
  if (!article) return {};
...
export default async function AppealLetterArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const article = getSeoArticle("appeal-letter", slug);
