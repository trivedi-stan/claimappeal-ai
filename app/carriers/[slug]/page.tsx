...
export async function generateStaticParams() {
  const articles = getArticlesByCategory("carriers");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = getSeoArticle("carriers", slug);
  if (!article) return {};
...
export default async function CarrierArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const article = getSeoArticle("carriers", slug);
